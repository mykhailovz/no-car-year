...
function normalizeCarModelTypeItem(item) {
  ...
  console.log(`NORM STR`, clonedStr);
  return clonedStr;
}
...
function normalizeCarModelItem(item) {
  let clonedStr = item.toLowerCase().trim().replace(/-/g, '');
  clonedStr = clonedStr.replace(/\d+$/, "");

  return clonedStr;
}

function findMatchingCarModel(carModelOptional, modelList, carCheck) {
  const matches = [];
  ...
  let carRegistrationYear = carCheck['carRegistrationYear'];
  let checkYearInc = carCheck['checkYearInc'];

  if (checkYearInc) {
    carRegistrationYear = Number(carRegistrationYear) - 1; // test part
  }

  if (matches.length > 1) {
    for (let i = 0; i < matches.length; i++) {
      if (matches[i].message.indexOf(carRegistrationYear) !== -1) { // test part
        return matches[i]; // test part
      } // test part
...
function normalizeCarModelTypeItem(item) {
  let clonedStr = item.toLowerCase().trim().replace(/-/g, '');
  clonedStr = clonedStr.replace(/\d+$/, ""); // test
